    ...
    <div className="p-8 max-w-4xl">
      ...
      <div className="flex items-start justify-between mb-8">
        ...
        <span className={`inline-flex items-center px-2.5 py-1 rounded text-xs font-medium ${
          cuenta.estado === 'activo' ? 'bg-success/10 text-success' :
          cuenta.estado === 'pausado' ? 'bg-warning/10 text-warning' :
          'bg-separator text-ink-muted'
        }`}>
          {cuenta.estado === 'activo' ? 'Activo' : cuenta.estado === 'pausado' ? 'Pausado' : 'Inactivo'}
        </span>
      </div>
          ...
          {integraciones.length > 0 ? (
            <div className="flex flex-wrap gap-2">
              {integraciones.map((key: string) => (
                <span key={key} className="px-2.5 py-1 bg-accent/8 text-accent text-xs font-medium rounded-md border border-accent/20">
